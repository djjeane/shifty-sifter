  ...
  async execute(interaction) {
    ...
    if (valid) {
      message.channel.send(
        "I have heard your message and will reply shortly my son."
      );
      const channels = message.guild.channels.filter((c) => c.type === "voice");
        ...
        for (const [memberID, member] of channel.members) {
          ...
          if (game != null) {
            ...
            if (game.name != "Custom Status" && game.name != "Spotify") {
              message.channel.send(
                `${member.user.tag} is playing ${game.name}`
              );
              ...
              if (index.getGames() != undefined) {
                if (index.getGames().includes(game.name)) {
                  ...
                  var xyz = message.guild.channels.filter(
                    (c) => c.type === "voice"
                  );
    ...
    if (index.getGames() == null) {
      message.channel.send(
        "No one is playing any games!!!!!! Dont waste my time man."
      );
    }
  },
};
...
exports.help = {
  ...
  description:
    "Take away all those loud pee sounds when the toilets are too close together.",
  usage: "sort",
};
