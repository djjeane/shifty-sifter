...
module.exports = async (client, message) => {
  ...
  if (cmd != null) {
    ...
    if (pointsReq != 0) {
      ...
      if (userPoints - pointsReq < 0) {
        message.reply(
          `This command costs ${pointsReq} points and you only have ${userPoints}.Make sure to SPIN THE WHEEL!!!!`
        );
        return;
      }
      ...
      message.reply(
        `You have paid ${pointsReq} points in order to use the ${
          cmd.help.name
        } command. Sifty now has ${points + pointsReq} points!`
      );
    }
  }
  ...
  if (cmd && !message.guild && cmd.conf.guildOnly)
    return message.channel.send(
      "This command is unavailable via private message. Please run this command in a Mercy Flush."
    );
